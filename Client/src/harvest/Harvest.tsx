...
const renderHarvest = (h: HarvestEvent | undefined) => {
  ...
  return h.type_name + " " + time.toLocaleDateString("nb-NO");
};
...
const getValidHarvestSelection = (
  h: HarvestSelector,
): ValidHarvest | undefined => {
...
export default function Harvest() {
    ...
    return (
      <FormControl fullWidth>
        ...
        <Select
          ...
          id="field-select"
          renderValue={() =>
            renderHarvest(harvestSelector().selectedHarvestEvent) ?? ""
          }
  ...
  const renderSelectOrAddHarvest = (harvests: () => HarvestEvent[]) => {
    return (
      ...
      >
        <Show when={harvests().length > 0}>{harvestSelect(harvests)}</Show>
      ...
      >
        <FormControl fullWidth>
          ...
          <Select
            ...
            error={showInvalid() && harvestType() == undefined}
            onChange={(value) => {
              const type = harvestTypes.data?.find(
                (h) => h.name === value.target.value,
              );
              if (type) {
                setEditHarvest({
                  ...editHarvest(),
  ...
  const updateHarvestInOuterState = (harvest: HarvestEvent) => {
    setHarvestSelector({
      ...harvestSelector(),
      harvests: harvestSelector().harvests?.map((h) =>
        h.id === harvest.id ? harvest : h,
      ),
      selectedHarvestEvent: harvest,
    });
  };
